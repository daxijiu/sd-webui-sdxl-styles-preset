...
class StyleSelectorXL(scripts.Script):
    ...
    def title(self):
        return "SDXL Styles Preset"

    def show(self, is_img2img):
        return scripts.AlwaysVisible

    def ui(self, is_img2img):
        with gr.Row(elem_id=f'{"img" if is_img2img else "txt"}2img_sdxl_style_preset', variant="compact"):
               style = gr.Dropdown(
                    label= 'SDXL Styles Preset', choices=self.styleNames, value='无(None)')
        return [style]

    def process(self, p, style):
        if style == '无(None)':
           return

        p.extra_generation_params["SDXL Styles Preset"] = True
